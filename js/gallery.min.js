...
function mosaic() {
	for (
		...
		GalleryWidth > 1500 && ((x = 6), (y = 4)),
			GalleryWidth > 2e3 && ((x = 7), (y = 5)),
			GalleryWidth < 700 && ((x = 4), (y = 2)),
			GalleryWidth < 500 && ((x = 2), (y = 1)),
			row % 2 == 0
...
function addGallery(e) {
	...
	$.each(i, function (e, t) {
		for (
			"tiles" == galleryStyle && (className = "tiles"),
				"full" == galleryStyle && (className = "full"),
				"center" == galleryStyle && (className = "center"),
				"Circles" == galleryStyle && (className = "Circles"),
				"mosaic" == galleryStyle && (className = "mosaic"),
				"mosaic" != className
					? $(".Gallery").append("<p>" + i.Entry.Title + "</p>")
					: $(".Gallery").hide(),
				length = Object.keys(i.Entry.Image).length,
				e = 0;
			e < length;
			e++
		)
			(x = AjaxCall(i, e)),
				1 == x
					? ($(".Gallery").append(
					  ...
					  ImageIndex++)
					: (i.Entry.Image[e].includes(".jpeg")
							? ((split = i.Entry.Image[e].split(".jpeg")),
							  (newURL = split[0] + "-preview.jpeg#preview"))
							: i.Entry.Image[e].includes(".jpg")
							? ((split = i.Entry.Image[e].split(".jpg")),
							  (newURL = split[0] + "-preview.jpg#preview"))
							: i.Entry.Image[e].includes(".png")
							? ((split = i.Entry.Image[e].split(".png")),
							  (newURL = split[0] + "-preview.png#preview"))
							: i.Entry.Image[e].includes(".webp") &&
							  ((split = i.Entry.Image[e].split(".webp")),
...
function AjaxCall(e, i) {
	return e.Entry.Image[i].includes(".jpeg#preview")
		? 2
		: e.Entry.Image[i].includes(".jpg#preview")
		? 2
		: e.Entry.Image[i].includes(".png#preview")
		? 2
		: e.Entry.Image[i].includes(".webp#preview")
		? 2
		: 1;
}
function checkPreviewState(e) {
	return (
		(newURL = e),
		e.includes("-preview.jpeg#preview")
			? ((split = e.split("-preview.jpeg#preview")), (newURL = split[0] + ".jpeg"))
			: e.includes("-preview.jpg#preview")
			? ((split = e.split("-preview.jpg#preview")), (newURL = split[0] + ".jpg"))
			: e.includes("-preview.png#preview")
			? ((split = e.split("-preview.png#preview")), (newURL = split[0] + ".png"))
			: e.includes("-preview.webp#preview") &&
			  ((split = e.split("-preview.webp#preview")),
			  (newURL = split[0] + ".webp")),
		newURL
	);
}
function addImageViewer() {
	0 == added &&
		...
		1 == enableViewOriginalPictureVal &&
			($(".GalleryViewer").append(
...
function closeViewer() {
	$("body").css("overflow", "unset"),
		...
		$(".ViewerImage").draggable({ disabled: !0 }),
		(zoomVal = 0),
		(monoImageView = 0),
		(monoImageViewURL = "");
}
...
function openViewer(e) {
	$("body").css("overflow", "hidden"), (viewerPosition = parseInt(e));
	var i = document.getElementById(e).src;
	(i = checkPreviewState(i)), $(".ViewerImage").attr("src", i);
	const t = new Image();
	t.onload = function () {
		(widthValue = this.width),
			(heightValue = this.height),
			(calc = widthValue / heightValue),
			calc < 1.25 || $(".ViewerImage").css("object-fit", "contain");
	};
	i = $(".ViewerImage").attr("src");
	(t.src = i),
		(t.onload = function () {
			$(".GalleryViewer").show();
		});
...
function left() {
	if (
		...
		var e = document.getElementById(viewerPosition).src;
		(e = checkPreviewState(e)), $(".ViewerImage").attr("src", e);
		const i = new Image();
		i.onload = function () {
			(widthValue = this.width),
				(heightValue = this.height),
		...
		(i.src = e),
			(i.onload = function () {
				"opacity" == transition &&
					...
					"slideAndZoom" == transition &&
						($(".ViewerImage").css("left", "-500px"),
						$(".ViewerImage").animate({ left: "0" }, 200),
						$(".ViewerImage").css("left", "500px"),
						$(".ViewerImage").css("opacity", "0.4"),
						$(".ViewerImage").css("width", "150px"),
						$(".ViewerImage").animate({ left: "0", opacity: "1", width: "80%" })),
					"slideZoom" == transition &&
						($(".ViewerImage").css("left", "-500px"),
						$(".ViewerImage").css("opacity", "0.4"),
						$(".ViewerImage").css("width", "150px"),
						$(".ViewerImage").animate({ left: "0", opacity: "1", width: "80%" })),
					"ZoominFast" == transition &&
						($(".ViewerImage").css("width", "0"),
						$(".ViewerImage").animate({ width: "100%" })),
					"bounce" == transition &&
...
function setGalleryTransition(e) {
	transition = e;
}
function openImage() {
			...
			(widthValue = this.width),
				...
				calc < 1.25
					? ($(".ViewerImage").animate(
					  ...
					  (zoomVal = 0))
					: ($(".ViewerImage").animate(
							{ width: "80%", "max-height": "80%", top: "0", right: "0", left: "0" },
							250
					  ),
					  $(".ViewerImage").css("height", "unset"),
					  (zoomVal = 0));
		};
		var e = $(".ViewerImage").attr("src");
		i.src = e;
	} else {
		const i = new Image();
		i.onload = function () {
			(widthValue = this.width),
				(heightValue = this.height),
				(calc = widthValue / heightValue),
				$(".ViewerImage").draggable({ disabled: !1, scroll: !1 }),
...
function enableExtraButtons() {
	(share = 1), (enableViewOriginalPictureVal = 1);
}
function Share() {
	$(".sharebuttons").toggle(),
...
function shareFacebook() {
	if (1 == monoImageView) {
		...
		e = document.getElementById(viewerPosition).src;
...
(added = 0),
	...
	$(document).ready(function () {
		...
		$(".ViewerImage").dblclick(function () {
			...
			$(".addToGallery").each(function () {
				$(this).attr("id", ImageIndex), ImageIndex++;
			}),
	...
	$(window).on("resize", function () {
		1 == AutoWidth &&
			((galleryWidth = $(".Gallery").width()),
			galleryWidth > 1920 && (galleryWidth4 = 300),
			galleryWidth < 1920 && (galleryWidth4 = galleryWidth / 5 - 20),
			galleryWidth < 1400 && (galleryWidth4 = galleryWidth / 4 - 20),
			galleryWidth < 1024 && (galleryWidth4 = galleryWidth / 3 - 20),
			galleryWidth < 786 && (galleryWidth4 = galleryWidth / 2 - 20),
			galleryWidth < 450 && (galleryWidth4 = galleryWidth / 1 - 20),
